...
export function PendleZapOutQuoteDisplay({
  tool,
  isOpen,
  onOpenChange
  ...
  const lpAmount = toolArgs.amountLpIn || '0'
  const outputAmount = Number(result.quote?.amountOut).toPrecision(6) || 0
  const priceImpact = Number(result.quote?.priceImpact) * 100 || 0
  const marketName = result.marketName || 'Unknown Market'
  const tokenOutName = result.tokenOutSymbol || 'Unknown Token'
