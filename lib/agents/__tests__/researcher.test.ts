...
describe('Researcher Agent Tool Registry Integration', () => {
  ...
  beforeEach(() => {
    ...
    mockRegistry = {
      getAllToolNames: jest.fn().mockReturnValue(['search', 'retrieve', 'wallet_balance', 'pendle_opportunities']),
      getToolNamesByCategory: jest.fn().mockImplementation((category) => {
        if (category === ToolCategory.WEB3) {
          return ['wallet_balance', 'pendle_opportunities']
        }
        return ['search', 'retrieve']
      }),
      getSupportedToolNames: jest.fn().mockReturnValue(['search', 'retrieve', 'wallet_balance']),
      getMaxSteps: jest.fn().mockImplementation((model, searchMode) => searchMode ? 10 : 5),
  ...
  test('uses tool registry for OpenAI models', () => {
    ...
    expect(result.experimental_activeTools).toEqual(['search', 'retrieve', 'wallet_balance'])
  ...
  test('uses web3 tools when search mode is disabled', () => {
    ...
    expect(result.experimental_activeTools).toEqual(['wallet_balance', 'pendle_opportunities'])
