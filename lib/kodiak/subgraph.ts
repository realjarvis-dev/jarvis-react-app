...
// Subgraph endpoint
export const SUBGRAPH_ENDPOINT = 'https://api.goldsky.com/api/public/project_clpx84oel0al201r78jsl0r3i/subgraphs/kodiak-v3-berachain-mainnet/latest/gn';

/**
 * Fetch vault data from the Kodiak Islands subgraph
 * @returns Promise with vault data
 */
export async function fetchVaultsFromSubgraph(): Promise<any[]> {
  try {
    const query = `
      query {
    ...
    console.error('Error fetching Kodiak vaults from subgraph:', error);
    return [];
  }
}
 ...
 * @returns Promise with vault data
 */
export async function fetchVaultByAddress(address: string): Promise<any | null> {
  try {
    const query = `
      query {
    ...
    console.error(`Error fetching Kodiak vault ${address} from subgraph:`, error);
    return null;
  }
}
